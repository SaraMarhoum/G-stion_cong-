...
$data .= '<h4><strong>Attestation de travail</strong></h4>';

$data .= '<p>La societé soussignée X dont le siège est à Y,</p><br />';
$data .= '<strong>Déclare</strong><br />';
$data .= '<p>que Mr/Mme.' .' '. $nom .' '. $prénom .' '. 'au CIN n° '.' '. $CIN .' '.'est 
enmployé(e) au sein de notre organisme en qualité de '.' '. $grade .' '.'dans le service de '.' '. $service .'.'.' </p><br />';

$data .= '<p>Toutes les indications et mentions portées sur la présente attestation sont certifiées exactes.</p><br />';

$mpdf->WriteHTML($data);

$mpdf->Output('AttestationTravail.pdf', 'D');
}
...
<body>
    ...
    <section class="contenu salarié">
        ...
        <div class="formaddsal">
            ...
            <form action="PDFattes.php" method="POST">
                <div class="form-row">
            ...
            <form class=" upd" method="post" action="PDFbdp.php">
                ...
                <p><strong>Attestation de travail de :  <?php echo $row['prénom']; ?>  <?php echo $row['nom']; ?></strong></P>
